...
#     else:
     await bot.answer_callback_query(call.id, text="ВЕДЕТСЯ АНАЛИЗ ОКРУЖЕНИЯ СИСТЕМЫ И СБОР ДАННЫХ!!!") # &&&-----| ВЫВОДИТ ВРЕМЕННО КОРОТКОЕ СООБЩЕНИЕ НА ЭКРАН!!! |------&&&
...
async def close_config_btn(call: types.CallbackQuery):
  if PASS:
    await bot.answer_callback_query(call.id, text='КОМАНДА ВЫПОЛНЕНА НА ПК!!!') # &&&-----| ВЫВОДИТ ВРЕМЕННО КОРОТКОЕ СООБЩЕНИЕ НА ЭКРАН!!! |------&&&
    today = datetime.now(tz)
    iTime = today.strftime("%H:%M:%S")
    iMsg = '☢️️  CONFIGs {' + iTime + '}'
